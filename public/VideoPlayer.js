...
class VideoPlayer
{
    ...
    async initialize(canvas, container)
    {
        ...
        for (const screenEntity of screens)
        {
            ...
            videoElement.imageEntity = children.find(e => e.getName() === 'image_holder');
        ...
        this.sdk.notifier.on('OnCamerasUpdated', async cameras =>
        {
            ...
            //const globalTransform = this.playerEntity.getGlobalTransform();
            ...
            for (const videoElement of this.videoElements)
            {
                ...
                if (isInside && !isVisible)
                {
                    ...
                    const {imageEntity} = videoElement
                    if(imageEntity)
                    {
                        await this.engineAPI.setEntityVisibility(imageEntity, false);
                    }
                ...
                else if (!isInside && isVisible)
                {
                    ...
                    if(imageEntity)
                    {
                        await this.engineAPI.setEntityVisibility(imageEntity, true);
                    }
    ...
    {
        const globalMatrix       = new this.THREE.Matrix4();
        const viewMatrix         = new this.THREE.Matrix4();
        const positionInGeometry = new this.THREE.Vector3();
        ...
        if (Math.abs(positionInGeometry.x) >= dimensions[0])
        {
            return false;
        }
        else if (Math.abs(positionInGeometry.y) >= dimensions[1])
        {
            return false;
        }
        else if (Math.abs(positionInGeometry.z) >= dimensions[2])
        {
            return false;
        }

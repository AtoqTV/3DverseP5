...
async function InitApp()
{
    ...
    let isPlaying = false;
    const audioPlayer = new Audio(getSoundURL(AppConfig.ambiantMusicUUID));
    const btnPlayMusic = document.getElementById('btn_play');
...
//--------------------------------------------------------------------------------------------------
function getSoundURL(uuid)
{
    return SDK3DVerse.webAPI.apiURL + '/Assets/Sounds/' + uuid + '?token=' + SDK3DVerse.webAPI.apiToken;
}
...
async function SpawnPlayer()
{
    ...
    playerTemplate.scene_ref.value  = AppConfig.characterControllerSceneUUID;

    // playerTemplate.local_transform = {
    //     position : [0, 0, 0],
    //     orientation : [0, 0, 0, 1],
    //     scale : [0.3, 0.3, 0.3]
    // }
